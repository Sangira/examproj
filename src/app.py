...
USERNAME_PASSWORD_PAIRS=[['svmukaeva', 'mukaeva_2607'], ['S', 'M']]
...
# Загрузка данных NASDAQ
nsdq = pd.read_csv('NASDAQcompanylist.csv')
nsdq.set_index('Symbol', inplace=True)

# Создание списка опций для Dropdown
options = []
...
def update_graph(n_clicks, stock_ticker, start_date, end_date):
    if not stock_ticker:
        return {'data': [{'x': [1, 2], 'y': [3, 1]}], 'layout': {'title': 'No Tickers Selected'}}
    ...
    for tic in stock_ticker:
        ...
        traces.append({'x': data.index, 'y': data['Close'], 'type': 'line', 'name': tic})
